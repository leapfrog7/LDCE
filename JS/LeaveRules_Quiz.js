function loadQuestions(){
    ...
    if(localStorage.getItem('sessionValidity')=='true'){
        ...
        hideEvaluation();
        hideOptions();
        const size_QB = Global_LeaveRules.length; //this measures the no. of row entries in the array
...
function loadQnA(randomArr){
    ...
    for(let i=0;i<randomArr.length-1;i++){
        console.log(i);
         Question_Arr.push(Global_LeaveRules[randomArr[i]][0]); // the random entry is then pushed into global question array
        //this stores the options for the particular question
         for(let j =0;j<4;j++){
            
            Selection_Options_Arr.push(Global_LeaveRules[randomArr[i]][j+1]);
        }
         
         //console.log(Question_Arr);

        Correct_Answers_Arr.push(Global_LeaveRules[randomArr[i]][5]); //this is the array which contains correct answers
        Explanation_Arr.push(Global_LeaveRules[randomArr[i]][6]);
