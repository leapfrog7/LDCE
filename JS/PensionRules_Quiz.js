...
function loadQnA(randomArr){
    ...
    for(let i=0;i<randomArr.length-1;i++){
        console.log(i);
         Question_Arr.push(Global_PensionRules[randomArr[i]][0]); // the random entry is then pushed into global question array
        //this stores the options for the particular question
         for(let j =0;j<4;j++){
            
            Selection_Options_Arr.push(Global_PensionRules[randomArr[i]][j+1]);
        }
         
         //console.log(Question_Arr);

        Correct_Answers_Arr.push(Global_PensionRules[randomArr[i]][5]); //this is the array which contains correct answers
        Explanation_Arr.push(Global_PensionRules[randomArr[i]][6]);
